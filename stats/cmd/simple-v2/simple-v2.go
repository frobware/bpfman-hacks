// Package main provides a simple BPF map reader for the stats-v2
// program.
//
...
//   - Map should be pinned at /sys/fs/bpf/stats-v2_map
//   - Link should be created between the program and tracepoint
//
// Use these bpftool commands to set up the environment:
//
//	bpftool prog load bpf/stats-v2.o /sys/fs/bpf/stats-v2_prog autoattach
//	bpftool map pin name stats_map /sys/fs/bpf/stats-v2_map
//
...
const mapPath = "/sys/fs/bpf/stats-v2_map"
...
func main() {
	...
	fmt.Println("Reading statistics (v2)...")
		...
		for iter.Next(&pid, &stats) {
			...
			if prev, exists := prevStats[pid]; exists {
				delta := Stats{
					...
					MajorFaults:     stats.MajorFaults - prev.MinorFaults,
				}

				if delta.ContextSwitches > 0 || delta.MajorFaults > 0 {
					fmt.Printf("PID %d: +CS: %d, +MF: %d\n",
						pid, delta.ContextSwitches, delta.MajorFaults)
				}
			}
		}
